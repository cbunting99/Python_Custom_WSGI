...
def run_demo_server():
    ...
    server = HighPerformanceWSGIServer(
        app=demo_app, host=host, port=port, workers=workers
    )
    ...
    print(
        f"🔍 Available endpoints: /, /api/status, /api/info, /health, /performance, /echo"
    )
    ...
    try:
        start_time = time.time()
        logging.info(
            f"Server starting at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        server.run()
