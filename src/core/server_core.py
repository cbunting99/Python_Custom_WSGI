...
class WSGIServer:
        ...
        *,
        ...
        if ssl_certfile and ssl_keyfile:
            ...
            self.ssl_context = create_ssl_context(
                cert_path, key_path, password=ssl_password, ciphers=ssl_ciphers
            )
            if enable_http2:
    ...
    async def shutdown(self, timeout: float = 30.0) -> None:
        ...
        if self._active_connections:
            default_logger.info(
                f"Waiting for {len(self._active_connections)} active connections to complete..."
            )
                ...
                if tasks:
                    ...
                    if pending:
                        default_logger.warning(
                            f"Force closing {len(pending)} connections that didn't complete in time"
                        )
                        for task in pending:
                            if not task.done():
                                task.cancel()
            ...
            try:
                ...
                while not self._shutdown_event.is_set():
                    try:
                        ...
                        connection_header = handler.headers.get(
                            "Connection", ""
                        ).lower()
