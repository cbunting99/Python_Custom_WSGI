...
class HTTPProtocolTests(unittest.TestCase):
        ...
        for method in methods:
            with self.subTest(method=method):
                ...
                if method == 'OPTIONS':
                    ...
                    self.assertTrue(
                        response.startswith(b'HTTP/1.1 204') or response.startswith(b'HTTP/1.1 200'),
                        f"OPTIONS request failed or returned unexpected status. Response: {response[:200]}"
                    )
                    if not self.captured_env:  # App was not called
                        continue
                
                self.assertIn('REQUEST_METHOD', self.captured_env, f"REQUEST_METHOD not in captured_env for {method}")
                self.assertEqual(self.captured_env['REQUEST_METHOD'], method, f"Wrong method {self.captured_env.get('REQUEST_METHOD')} != {method}")
                
                if method == 'HEAD':
                    self.assertNotIn(b'Test Response', response, "Body should not be in HEAD response")
                    self.assertTrue(response.endswith(b'\r\n\r\n'), "HEAD response format incorrect")
                elif method != 'OPTIONS':  # OPTIONS might not return 'Test Response' if not handled by app
                    self.assertIn(b'Test Response', response, "Body missing from response")
    ...
    def test_keep_alive(self):
        ...
        self.assertTrue(
            b'connection: keep-alive' in response_headers or b'Connection: keep-alive' in response_headers,
        ...
        self.assertTrue(
            b'connection: close' in response_headers or b'Connection: close' in response_headers,
    ...
    def test_malformed_requests(self):
        ...
        self.assertTrue(response_invalid_line.startswith(b'HTTP/1.1 400'), "Failed: Invalid request line")
    ...
    def test_error_handling_in_app(self):
        ...
        request = (
            b'GET /error_test HTTP/1.1\r\n'
            b'Host: example.com\r\n'
            b'\r\n'
        )
        response = self._run_raw_request(request, handler_override=error_handler)
        
        self.assertTrue(response.startswith(b'HTTP/1.1 500'), "Application error did not result in 500 status")
        self.assertIn(b'Internal Server Error', response, "Application error response body incorrect")

    def _run_raw_request(self, request_data: bytes, handler_override: Optional[WSGIHandler] = None) -> bytes:
